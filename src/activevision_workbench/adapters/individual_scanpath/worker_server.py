...
class WorkerBackend(Protocol):
    def predict(self, request: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, Any]]: ...
...
def serve(factory: Callable[[dict[str, Any]], WorkerBackend]) -> int:
    ...
    atomic_write_json(Path(args.ready), {"schema_version": 1, "status": "ready"})
    for line in sys.stdin:
        ...
        try:
            ...
            if not isinstance(command, dict) or set(command) != {"request_path", "response_path"}:
            ...
            request = _read(Path(_text(command["request_path"], "request_path")), "worker request")
            ...
            for raw in samples:
                ...
                atomic_write_json(
                    native_path,
                    {
                        ...
                        "model": "individualscanpath",
                        "variant": config["variant"],
                        "upstream_commit": config["upstream_commit"],
                        "observer_mapping_sha256": config["observer_mapping_sha256"],
                        "native_artifact_policy": config["native_artifact_policy"],
                        "sample": raw,
                    },
                )
                sample = dict(raw)
...
def _validate_config(config: dict[str, Any]) -> None:
    expected = {
        "schema_version", "variant", "upstream_root", "upstream_commit",
        "checkpoint_path", "checkpoint_sha256", "observer_mapping_sha256",
        "device", "min_fixations", "model_max_fixations", "input_width",
        "input_height", "map_width", "map_height", "subject_count",
        "embedding_dim", "native_artifact_policy", "config_sha256", "fake_behavior",
    }
    ...
    if config["variant"] != INDIVIDUAL_SCANPATH_CHENLSTM_OSIE_VARIANT:
        raise ValueError("worker config variant is unsupported")
    for name in (
        "upstream_root", "upstream_commit", "checkpoint_path", "checkpoint_sha256",
        "observer_mapping_sha256", "device", "native_artifact_policy",
        "config_sha256", "fake_behavior",
    ):
        _text(config[name], f"worker config.{name}")
    if config["native_artifact_policy"] != INDIVIDUAL_SCANPATH_NATIVE_ARTIFACT_POLICY:
        raise ValueError("worker native artifact policy is invalid")
    for name in (
        "min_fixations", "model_max_fixations", "input_width", "input_height",
        "map_width", "map_height", "subject_count", "embedding_dim",
    ):
        if type(config[name]) is not int or config[name] < 0:
            raise ValueError(f"worker config.{name} must be a non-negative integer")


def _validate_request(request: dict[str, Any], config: dict[str, Any]) -> None:
    expected = {
        "schema_version", "request_id", "image_ref", "image_width", "image_height",
        "observer_id", "model_observer_index", "sample_specs", "max_fixations",
        "native_output_dir",
    }
    ...
    for name in ("request_id", "image_ref", "observer_id", "native_output_dir"):
        _text(request[name], f"worker request.{name}")
    for name in ("image_width", "image_height", "max_fixations", "model_observer_index"):
        if type(request[name]) is not int or request[name] < 0:
            raise ValueError(f"worker request.{name} is invalid")
    if request["model_observer_index"] >= config["subject_count"]:
        raise ValueError("worker request observer index is out of range")
    if not (1 <= request["max_fixations"] <= config["model_max_fixations"]):
    ...
    for index, spec in enumerate(specs):
        ...
        _keys(spec, {"sample_id", "sample_index", "seed"}, f"sample_specs[{index}]")
        _text(spec["sample_id"], f"sample_specs[{index}].sample_id")
        if type(spec["sample_index"]) is not int or type(spec["seed"]) is not int:
            raise ValueError(f"sample_specs[{index}] index and seed must be integers")
...
def _keys(value: Mapping[str, object], expected: set[str], label: str) -> None:
    if set(value) != expected:
        ...
        detail = f"missing field '{sorted(missing)[0]}'" if missing else f"unknown field '{sorted(unknown)[0]}'"
