...
def validate_response(
    ...
    *,
    ...
    root = _object(value, "TPPGazeWorkerResponse")
    _exact_keys(
        root,
        required={"schema_version", "status", "request_id", "samples", "metadata"},
        label="TPPGazeWorkerResponse",
    )
    if root["schema_version"] != PROTOCOL_VERSION:
        raise AdapterOutputError("TPPGazeWorkerResponse.schema_version: expected 1")
    if root["request_id"] != request_id:
        raise AdapterOutputError(
            "TPPGazeWorkerResponse.request_id does not match the request"
        )
    if root["status"] != "ok":
        raise AdapterOutputError("TPPGazeWorkerResponse.status: expected 'ok'")

    metadata = _object(root["metadata"], "TPPGazeWorkerResponse.metadata")
    _exact_keys(
        metadata,
        required={
            "checkpoint_sha256",
        ...
        label="TPPGazeWorkerResponse.metadata",
    )
    for name in (
        "checkpoint_sha256",
        "model_config_sha256",
        ...
        _string(metadata[name], f"TPPGazeWorkerResponse.metadata.{name}")
    _number(metadata["temperature"], "TPPGazeWorkerResponse.metadata.temperature")
    if metadata["temperature"] <= 0:
        raise AdapterOutputError(
            "TPPGazeWorkerResponse.metadata.temperature must be positive"
        )
    if metadata["temperature_source"] not in {
        "model_config",
        "adapter_override",
    }:
        raise AdapterOutputError(
            "TPPGazeWorkerResponse.metadata.temperature_source is unknown"
        )
    for name in ("image_width", "image_height"):
        _integer(metadata[name], f"TPPGazeWorkerResponse.metadata.{name}", minimum=1)

    raw_samples = root["samples"]
    if not isinstance(raw_samples, list):
        raise AdapterOutputError("TPPGazeWorkerResponse.samples must be an array")
    if len(raw_samples) != sample_count:
        raise AdapterOutputError(
            f"TPP-Gaze worker returned {len(raw_samples)} samples; "
    ...
    for position, raw_sample in enumerate(raw_samples):
        label = f"TPPGazeWorkerResponse.samples[{position}]"
        ...
        for event_index, raw_event in enumerate(events):
            _validate_event(raw_event, f"{label}.events[{event_index}]")
        reason = _string(sample["stopping_reason"], f"{label}.stopping_reason")
        if reason not in {"time_horizon", "max_fixations", "model_stop"}:
            raise AdapterOutputError(f"{label}.stopping_reason is unknown")
    ...
    if indices != set(range(sample_count)):
        raise AdapterOutputError(
            "TPPGazeWorkerResponse sample indices must be exactly "
            f"0..{sample_count - 1}"
        )
...
def _validate_event(value: object, label: str) -> None:
    ...
    _exact_keys(
        ...
        required={
            ...
            "mark_x_native",
            "mark_y_native",
            "inter_event_time_native",
            "arrival_time_native",
            "model_inter_event_time_s",
            "model_arrival_time_s",
            "included",
        },
        label=label,
    )
    for name in ("x_px", "y_px", "mark_x_native", "mark_y_native"):
    ...
    if type(event["included"]) is not bool:
        raise AdapterOutputError(f"{label}.included must be a boolean")
